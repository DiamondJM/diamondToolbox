function [coordidx,D,scoords,vORr,fastmarching_outputs,subsurface_outputs] = surfing_circleROI_custom(coords,faces,centervertex_idx,radius,distancemetric,n2f) 
...
%   vORr:       Mike has no idea
%   fastmarching_outputs: a struct with the outputs from fastmarching (D, S, and Q)
%   subsurface_outputs: a struct with the outputs from subsurface (sv, sf, si, vidxs, fidxs);
%   
%
...
while true
    switch distancemetric
        ...
        case 'geodesic'
            ...
            % used the code below
            [sv, sf, si,vidxs, fidxs]=surfing_subsurface_custom(coords, faces, centervertex_idx, radius(1), n2f); % construct correct sub surface
            subsurface_outputs.sv = sv;
            subsurface_outputs.sf = sf;
            subsurface_outputs.si = si;
            subsurface_outputs.vidxs = vidxs;
            subsurface_outputs.fidxs = fidxs;
            
            % this requires the Fast Marching toolbox (Peyre)
            [D,S,Q] = perform_fast_marching_mesh(sv, sf, si);     
            fastmarching_outputs.D = D;
            fastmarching_outputs.S = S;
            fastmarching_outputs.Q = Q;
