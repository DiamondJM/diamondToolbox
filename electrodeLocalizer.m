...
% See also: sourceLocalizer, create_surf, suma
    ...
    methods
        ...
        function manualCoregisterCT(self, varargin)
            ...
            if exist(ctWork, 'file') ~= 2, copyfile(ctNii, ctWork); end
            if exist(mrWork, 'file') ~= 2
                fprintf('[Stage 5] Resampling MR to RAI orientation...\n');
                cmd = sprintf('"%s/3dresample" -orient RAI -inset "%s" -prefix "%s" -overwrite', ...
                    self.afniBin, mrNii, mrWork);
                [cst, ~] = unix(cmd);
            ...
            fprintf('  MRI:  %s\n', mrWork);
            fprintf('  CT:   %s\n', ctWork);
            % Use open -a without --args to avoid Qt/macOS 15 crash (QAppleKeyMapper).
            % FreeView receives files via a temporary command file instead.
            unix(sprintf('open -a "%s"', fvApp));

            % Wait for user
            msg = { ...
                'FreeView should be opening.  If it is not open yet, wait a moment.', ...
                '', ...
                'Load the two files via File > Load Volume:', ...
                sprintf('  MRI:  %s', mrWork), ...
                sprintf('  CT:   %s', ctWork), ...
                '', ...
                'To align the CT to the MRI:', ...
                '  1. Click on the CT volume in the Volumes panel', ...
                '  2. Set CT colormap to "Heat" and opacity ~0.5', ...
                '  3. Go to Tools > Transform Volume', ...
                '  4. Use the sliders to align CT skull to MRI skull', ...
                '  5. When aligned, go to File > Save Volume As...', ...
                sprintf('  6. Save as:  ct_manual_coreg.nii'), ...
                sprintf('     in folder: %s', workDir), ...
                '', ...
                'Click OK when you have saved the file.'};
            uiwait(msgbox(msg, 'Manual CT-MR Coregistration', 'help', 'modal'));

            assert(exist(ctCoregNii,'file')==2, ...
                '[manualCoregisterCT] ct_manual_coreg.nii not found in:\n  %s\nRe-run and save the file.', workDir);
