...
% See also: sourceLocalizer, create_surf, suma
    ...
    methods (Access = private)
        ...
        function dlg = localizationSetupDialog(self)
            ...
            % are missing.  Presents 5 required files with status indicators
            % and inline descriptions; lets the user choose Create, Import,
            % or Cancel.  In Import mode path fields + browse buttons appear
            % for each missing file; confirming copies files into place.
            %
            % Returns struct:
            %   .action  — 'create' | 'import' | 'cancel'
            ...
            descs = { ...
                ...
                sprintf(['Left pial-outer-smoothed surface (GIFTI).\n' ...
                    ...
                    'Required for snapping subdural contacts to cortex.\n' ...
            ...
            % ---- theme ---------------------------------------------------
            BG    = [0.13 0.13 0.13];
            ROWBG = [0.18 0.18 0.18];
            SELBG = [0.20 0.27 0.38];
            FG    = [0.92 0.92 0.92];
            DIM   = [0.52 0.52 0.52];
            GRN   = [0.35 0.85 0.35];
            RED   = [0.88 0.35 0.35];
            ...
            HDR_H   = 56;
            DESC_H  = 100;
            IMP_H   = PAD + 18 + N * 40 + PAD;   % import path section
            BTN_H   = 52;
            SEP     = 8;
            totalH  = HDR_H + N*ROW_H + SEP + DESC_H + SEP + IMP_H + SEP + BTN_H;

            % y positions (bottom = 0)
            btnY    = 8;
            impY    = btnY  + BTN_H  + SEP;
            descY   = impY  + IMP_H  + SEP;
            rowsY   = descY + DESC_H + SEP;   % bottom of first (topmost) row

            % ---- state stored in figure appdata --------------------------
            dlg = struct('action', 'cancel');
            setappdata(0, 'eloc_dlg_result', dlg);
            inImport = false;
            ...
            % ---- header --------------------------------------------------
            uicontrol(fig, 'Style','text', ...
                'String', sprintf('Required localization files not found for  %s', self.subj), ...
                'ForegroundColor', FG, 'BackgroundColor', BG, ...
                'FontSize', 12, 'FontWeight', 'bold', ...
                'HorizontalAlignment', 'left', ...
                'Position', [PAD rowsY+N*ROW_H+10 W-PAD*2 22]);
            uicontrol(fig, 'Style','text', ...
                'String', 'Click a row to see details. Choose  Create  to run the pipeline or  Import  to copy existing files.', ...
                'ForegroundColor', DIM, 'BackgroundColor', BG, ...
                'FontSize', 9, ...
                'HorizontalAlignment', 'left', ...
                'Position', [PAD rowsY+N*ROW_H-6 W-PAD*2 18]);

            % ---- file rows -----------------------------------------------
            rowPanels = cell(N, 1);
            for i = 1:N
                ry = rowsY + (N - i) * ROW_H;
                rowPanels{i} = uipanel(fig, ...
                    'BackgroundColor', ROWBG, ...
                    'BorderType', 'none', ...
                    'Position', [0 ry W ROW_H]);

                iconStr = '✓'; iconClr = GRN;
                if ~present(i), iconStr = '✗'; iconClr = RED; end

                uicontrol(rowPanels{i}, 'Style','text', ...
                    'String', iconStr, ...
                    'ForegroundColor', iconClr, 'BackgroundColor', ROWBG, ...
                    'FontSize', 13, 'FontWeight', 'bold', ...
                    'HorizontalAlignment', 'center', ...
                    'Position', [6 4 22 ROW_H-8]);

                uicontrol(rowPanels{i}, 'Style','text', ...
                    'String', names{i}, ...
                    'ForegroundColor', FG, 'BackgroundColor', ROWBG, ...
                    'FontSize', 11, 'HorizontalAlignment', 'left', ...
                    'Position', [34 4 W-34-80-PAD ROW_H-8]);

                stClr = DIM; stStr = 'Missing';
                if present(i), stClr = GRN; stStr = 'Present'; end
                uicontrol(rowPanels{i}, 'Style','text', ...
                    'String', stStr, ...
                    'ForegroundColor', stClr, 'BackgroundColor', ROWBG, ...
                    'FontSize', 10, 'HorizontalAlignment', 'right', ...
                    'Position', [W-80-PAD 4 80 ROW_H-8]);

                % make entire row clickable
                ri = i;
                set(rowPanels{i}, 'ButtonDownFcn', @(~,~) selectRow(ri));
                kids = get(rowPanels{i}, 'Children');
                for jj = 1:numel(kids)
                    set(kids(jj), 'ButtonDownFcn', @(~,~) selectRow(ri));
                end
            end
            ...
            % ---- import section (always in layout, initially hidden) -----
            hImpPanel = uipanel(fig, ...
                'BackgroundColor', BG, ...
                'BorderType', 'none', ...
                'Visible', 'off', ...
                'Position', [0 impY W IMP_H]);
            uicontrol(hImpPanel, 'Style','text', ...
                'String', 'Source paths for missing files:', ...
                'ForegroundColor', DIM, 'BackgroundColor', BG, ...
                'FontSize', 9, 'HorizontalAlignment', 'left', ...
                'Position', [PAD IMP_H-PAD-14 300 14]);

            importEdits = cell(N, 1);
            for i = 1:N
                iy = IMP_H - PAD - 18 - (i-1)*40 - 20;
                uicontrol(hImpPanel, 'Style','text', ...
                    'String', names{i}, ...
                    'ForegroundColor', electrodeLocalizer.ternary(present(i), DIM, FG), ...
                    'BackgroundColor', BG, ...
                    'FontSize', 10, 'HorizontalAlignment', 'left', ...
                    'Position', [PAD iy+22 240 16]);
                importEdits{i} = uicontrol(hImpPanel, 'Style','edit', ...
                    'String', electrodeLocalizer.ternary(present(i), dests{i}, ''), ...
                    'Enable', electrodeLocalizer.ternary(present(i), 'inactive', 'on'), ...
                    'ForegroundColor', electrodeLocalizer.ternary(present(i), DIM, FG), ...
                    'BackgroundColor', electrodeLocalizer.ternary(present(i), [0.16 0.16 0.16], EDBG), ...
                    'FontSize', 10, 'HorizontalAlignment', 'left', ...
                    'Position', [PAD iy W-PAD*2-50 22]);
                ri = i;
                uicontrol(hImpPanel, 'Style','pushbutton', ...
                    'String', '...', ...
                    'Enable', electrodeLocalizer.ternary(present(i), 'off', 'on'), ...
                    'ForegroundColor', FG, 'BackgroundColor', BTN, ...
                    'FontSize', 11, ...
                    'Position', [W-PAD-44 iy 44 24], ...
                    'Callback', @(~,~) browseFile(ri));
            end

            % ---- action buttons ------------------------------------------
            bW = 130; bH = 34;
            hBtnCreate = uicontrol(fig, 'Style','pushbutton', ...
                'String', 'Create', ...
                'ForegroundColor', FG, 'BackgroundColor', [0.18 0.42 0.18], ...
                'FontSize', 11, 'FontWeight', 'bold', ...
                'Position', [PAD btnY bW bH], ...
                'Callback', @cbCreate);
            hBtnImport = uicontrol(fig, 'Style','pushbutton', ...
                'String', 'Import', ...
                'ForegroundColor', FG, 'BackgroundColor', BTN, ...
                'FontSize', 11, ...
            ...
            function selectRow(idx)
                for k = 1:N
                    bg = electrodeLocalizer.ternary(k == idx, SELBG, ROWBG);
            ...
            function cbCreate(~,~)
                r = struct('action','create');
                setappdata(0, 'eloc_dlg_result', r);
                delete(fig);
            end
            ...
            function cbImport(~,~)
                if ~inImport
                    inImport = true;
                    set(hImpPanel, 'Visible', 'on');
                    set(hBtnImport, 'String', 'Copy & Continue');
                    set(hBtnCreate, 'Enable', 'off');
                else
                    % Perform copies for missing files.
                    if ~exist(talDir,  'dir'), mkdir(talDir);  end
                    if ~exist(sumaDir, 'dir'), mkdir(sumaDir); end
                    for k = 1:N
                        if present(k), continue; end
                        src = strtrim(get(importEdits{k}, 'String'));
                        if isempty(src), continue; end
                        if ~exist(src, 'file')
                            warndlg(sprintf('File not found:\n%s', src), names{k});
                            return;
                        end
                        try
            ...
            function cbCancel(~,~)
                r = struct('action', 'cancel');
                setappdata(0, 'eloc_dlg_result', r);
                if ishandle(fig), delete(fig); end
            end

            function browseFile(idx)
                if strcmp(names{idx}, 'leads.csv')
                    [f, d] = uigetfile('*.csv', sprintf('Select %s', names{idx}));
                else
                    [f, d] = uigetfile('*.gii', sprintf('Select %s', names{idx}));
                end
                if ~isequal(f, 0)
                    set(importEdits{idx}, 'String', fullfile(d, f));
                end
            end
        end
